<#
.Description: Collects Brave Browser Logs and Forensic Artifacts for Browser Forensics.
...
.Example:
    .\BraveArtifacts.ps1 -Username "Belk0ruh"
    .\BraveArtifacts.ps1 -Username "Belk0ruh" -CollectAllArtifacts
    .\BraveArtifacts.ps1 -AllUsers
    .\BraveArtifacts.ps1 -AllUsers -CollectAllArtifacts
    .\BraveArtifacts.ps1 -Username "Belk0ruh" -OutputDir "C:\IR\Export"
#>
...
if (-not $OutputDir) {
    $OutputDir = Join-Path -Path (Get-Location) -ChildPath "BraveBrowserArtifacts"
}
$HistoryFolder = Join-Path -Path $OutputDir -ChildPath "Browsers\Brave"
New-Item -Path $HistoryFolder -ItemType Directory -Force | Out-Null

Write-Host "Collecting Brave Browser artifacts..." -ForegroundColor Cyan
...
foreach ($userDir in $userDirs) {
    ...
    $braveBasePaths = @(
        "$($userDir.FullName)\AppData\Local\BraveSoftware\Brave-Browser\User Data",
        "$($userDir.FullName)\AppData\Local\BraveSoftware\Brave-Browser-Beta\User Data",
        "$($userDir.FullName)\AppData\Local\BraveSoftware\Brave-Browser-Nightly\User Data"
    )

    foreach ($basePath in $braveBasePaths) {
        ...
        foreach ($profile in $profileDirs) {
            ...
            if ($CollectAllArtifacts) {
                Write-Host "Collecting ALL Brave files from $($profile.FullName)" -ForegroundColor Yellow
                ...
                Write-Host "Collecting targeted Brave forensic artifacts from $($profile.FullName)"
...
Write-Host "Brave Browser artifacts collection completed ($collectedCount profile(s) processed). Output folder: $OutputDir" -ForegroundColor Green
